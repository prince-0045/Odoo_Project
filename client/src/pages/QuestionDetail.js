...
import { FiMessageSquare, FiEye, FiClock, FiUser, FiTag, FiThumbsUp, FiThumbsDown, FiCheck, FiSend } from 'react-icons/fi';
...
const QuestionDetail = () => {
  ...
  const [loadingComments, setLoadingComments] = useState({}); // { [answerId]: boolean }
  ...
  const fetchComments = async (answerId) => {
    setLoadingComments((prev) => ({ ...prev, [answerId]: true }));
    try {
      const res = await answerService.getComments(answerId);
      setComments((prev) => ({ ...prev, [answerId]: res.comments || [] }));
    } catch (e) {
      setComments((prev) => ({ ...prev, [answerId]: [] }));
    } finally {
      setLoadingComments((prev) => ({ ...prev, [answerId]: false }));
    }
  };
  ...
  const handlePostComment = async (answerId) => {
    ...
    try {
      await answerService.postComment(answerId, content);
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-2">
                        <button
                          ...
                          onClick={() => fetchComments(answer._id)}
                          type="button"
                        >
                          {comments[answer._id]?.length ? 'Hide Comments' : 'Show Comments'}
                        </button>
                        {loadingComments[answer._id] && <div className="text-xs text-gray-400">Loading comments...</div>}
                        {comments[answer._id] && comments[answer._id].length > 0 && (
                          <div className="space-y-2 mt-2">
                            {comments[answer._id].map((comment, idx) => (
                              <div key={comment._id || idx} id={`answer-${answer._id}-comment-${comment._id}`} className="bg-gray-50 rounded p-2 text-sm">
                                <span className="font-semibold">{comment.author?.username || 'User'}:</span> {comment.content}
                                {comment.mentions && comment.mentions.length > 0 && (
                                  <span className="ml-2 text-xs text-blue-500">(mentions: {comment.mentions.map(u => u.username).join(', ')})</span>
                                )}
                              </div>
                            ))}
                          </div>
                        ...
                        {currentUser && (
                          <div className="flex items-center mt-2">
                            <input
                              ...
                              onChange={e => handleCommentInput(answer._id, e.target.value)}
                              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handlePostComment(answer._id); } }}
                            />
                            <button
                              className="ml-2 text-blue-600 hover:text-blue-800"
                              onClick={() => handlePostComment(answer._id)}
                              type="button"
                            >
                              <FiSend />
                            </button>
